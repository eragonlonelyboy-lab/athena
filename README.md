...
**Claude agrees with you by default. SESHAT holds a trial.**

SESHAT: Sharp Evaluation, Structured Honesty, Adversarial Truth.

Seshat, Egyptian goddess of wisdom and writing, Foremost of the House of Books: she recorded the deeds of kings and the verdicts of the gods, and nothing entered the record falsely. SESHAT does the same for your decisions: any idea, any question, tried by adversarial counsel, judged with one decisive verdict, and written on the record so you can check how it aged.

## Before / after

**Without SESHAT**, you ask your AI about your startup idea:
...
Recorded: .seshat/decisions/2026-07-05-freelancer-tool.md
```

And next time you invoke SESHAT, one line: *"Open verdict on file: did you run the 10-DM test?"* The loop nobody closes.
...
- **Decision records:** every verdict on file as git-diffable markdown with scores, reasoning, and an open follow-up block. SESHAT asks about untested verdicts exactly once, never nags.
...
```powershell
git clone https://github.com/eragon/seshat.git; cd seshat
New-Item -ItemType Directory -Force "$env:USERPROFILE\.claude\skills\seshat" | Out-Null
Copy-Item SKILL.md "$env:USERPROFILE\.claude\skills\seshat\"; Copy-Item -Recurse -Force references "$env:USERPROFILE\.claude\skills\seshat\"
```
...
```bash
git clone https://github.com/eragon/seshat.git && cd seshat
mkdir -p ~/.claude/skills/seshat && cp SKILL.md ~/.claude/skills/seshat/ && cp -r references ~/.claude/skills/seshat/
```

Then in any conversation: `/seshat your idea`, or just "pressure-test this," "should I do A or B," "council this." Zero config; decision records appear on the first verdict. If anything breaks, open your agent in this repo and say: "read CLAUDE.md and set SESHAT up for me."

## Ecosystem

SESHAT is part of the Demiurge line: mythology-named tools that keep AI agents honest.
...
| **SESHAT** | **Foremost of the House of Books** | **decision trials with verdicts on the record** |

Natural pairs: a verdict that ages badly becomes a CHIRON rule; a decision worth announcing goes through VERITAS.

## Star this repo

SESHAT costs you one hard conversation before you build. The alternative costs you the build. A star costs one click and tells us to keep going. Fair trade.
