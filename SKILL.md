...
- **Extra lenses** (`references/harvested-lenses.md`): 12 distilled decision, adversarial, and ideation lenses (weighted scoring matrix, the fear/desire beneath a decision, stack-choice as a decision, SCAMPER, diverge/converge brainstorm, first-principles rebuild, and more). Each is tagged as adding to or overlapping the core modes, so ATHENA extends without duplicating itself.
