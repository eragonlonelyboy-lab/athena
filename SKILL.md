...
- **Extra lenses** (`references/harvested-lenses.md`): 13 distilled decision, adversarial, and ideation lenses, the full harvest of all 13 source prompts (weighted scoring matrix, option comparison with best-for verdicts, the fear/desire beneath a decision, stack-choice as a decision, SCAMPER, diverge/converge brainstorm, first-principles rebuild, and more). Each is tagged as adding to or overlapping the core modes, so ATHENA extends without duplicating itself.
...
- **The easy verdict earns the hardest attack.** A conclusion that arrives quickly and feels obvious is not thereby correct; ease of arrival is no evidence at all. When the panel converges in the first minutes, that convergence is itself a finding to attack: give the obvious option the most hostile persona time, not the least.
