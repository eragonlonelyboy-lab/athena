...
A war council is five structured perspectives and a synthesis, run by language models. It can be wrong, and sometimes will be. What IS checkable: every verdict goes on file with its reasoning, scores, and a follow-up block, so you can audit how the council's calls age against reality. Trust the record, not the confidence score.

## The research honesty, in full
...
Five parallel subagents plus a judge is the expensive path (typically several minutes and meaningful token spend; the Researcher persona also runs web searches). Counsel mode costs a normal conversation. Do not council a question that one sharp lens answers.
...
Decision records close their loop when you report what happened. If you never run the 48-hour tests and never answer the one-line follow-up, you have a folder of well-formatted opinions. The single-ask, never-nag design is deliberate (trust over pressure), and its cost is that discipline stays yours.
